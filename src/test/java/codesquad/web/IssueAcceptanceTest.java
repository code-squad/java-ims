...
public class IssueAcceptanceTest extends AcceptanceTest {
	...
	public void detail() throws Exception {
		...
		Long issueId = Optional.ofNullable(findIssueId("detail 제목"))
				.orElseThrow(() -> new MyExeption("해당하는 이슈를 찾지 못했습니다."));
	...
	public void update() throws Exception {
		...
		HtmlFormDataBuilder dataBuilder = HtmlFormDataBuilder.urlEncodedForm().addParameter("title", "update후 제목")
				.addParameter("contents", "update후 내용");
		HttpEntity<MultiValueMap<String, Object>> request = dataBuilder.build();
		basicAuthTemplate().put(String.format("/issues/%d", findIssueId("update전 제목")), request);

		ResponseEntity<String> response2 = template()
				.getForEntity(String.format("/issues/%d", findIssueId("update후 제목")), String.class);
		assertThat(response2.getStatusCode(), is(HttpStatus.OK));
		log.info("body : {}", response2.getBody());
	...
	public void update_no_auth() throws Exception {
		...
		HtmlFormDataBuilder dataBuilder = HtmlFormDataBuilder.urlEncodedForm().addParameter("title", "updatefail 후 제목")
				.addParameter("contents", "updatefail후 내용");
		HttpEntity<MultiValueMap<String, Object>> request = dataBuilder.build();
		...
		ResponseEntity<String> response2 = template()
				.getForEntity(String.format("/issues/%d", findIssueId("updatefail 후 제목")), String.class);
